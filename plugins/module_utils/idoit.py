    ...
    def run(self):
        ...
        if self.idoit_spec['single_value_cat']:
            ...
            if self.params['id'] and self.params['search_by_fields']:
                self.fail_json(msg='You can only specify "id" or "search_by_fields" not both')
            if self.params['id']:
                old_idoit_data=self.idoit_cat_api.read_category_by_id(self.params['obj_id'],self.params['id'])
            elif self.params['search_by_fields']:
                cat_ids=[]
                result=self.idoit_cat_api.read_categories(self.params['obj_id'])
                ...
                if len(cat_ids)>1:
                    self.fail_json(msg='More than one category with the same search_by_fields (ids:%s)' % ",".join(list(map(str,cat_ids))))
                elif len(cat_ids)==0:
                    old_idoit_data=None
                elif len(cat_ids)==1:
        ...
        if result['changed']:
            if self.check_mode:
                ...
                if 'id' in self.params.keys() and self.params['id'] is not None:
                    ...
                    r=self.idoit_cat_api.save_category(self.params['obj_id'], idoit_new_data)
            result['return']=r
        if self._diff:
            result["diff"] = {
...
class IdoitCategoryInfoModule(AnsibleModule):
    ...
    def run(self):
        ...
        if self.idoit_spec['single_value_cat']:
            ...
            old_idoit_data_arr = self.idoit_cat_api.read_categories(self.params['obj_id'])
            data=[]
