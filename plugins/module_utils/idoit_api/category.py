...
class IDoitCategory(IDoitApiBase):
    ...
    def conv_array_field(self,fieldname,data,ref_field):
        ...
        if len(data[fieldname])!=1:
            raise Exception('Field "%s" has more than one entry %s' %(fieldname,json.dumps(data)))
    ...
    def convert_field(self, fieldname, data):
        ...
        if fieldname in data.keys():
            ...
            try:
                ...
                raise Exception('Wrong conversion ',self.obj_type, fieldname,field,data)
            raise Exception('Unknwown data_type/ ui_type',self.obj_type, fieldname,field,data)
